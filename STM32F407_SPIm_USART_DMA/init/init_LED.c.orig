...
void Config_LED()
{
		...
		GPIOA->MODER |= 0x01 << GPIO_MODER_MODER6_Pos;//режим на выход
		GPIOA->OSPEEDR |= 0 << GPIO_OSPEEDR_OSPEED6_Pos;//малая скорость
		GPIOA->PUPDR |= 0x01 << GPIO_PUPDR_PUPD6_Pos;// установка подтяжки к + (1)
		GPIOA->OTYPER |= 1 << GPIO_OTYPER_OT6_Pos;//Выход с открытым стоком 
		
		GPIOA->MODER |= 0 << GPIO_MODER_MODER7_Pos;// сброс
		GPIOA->MODER |= 0x01 << GPIO_MODER_MODER7_Pos;//режим на выход
		GPIOA->OSPEEDR |= 0 << GPIO_OSPEEDR_OSPEED7_Pos;//малая скорость
		GPIOA->PUPDR |= 0x01 << GPIO_PUPDR_PUPD7_Pos;// установка подтяжки к + (1)
